...
use constant FILE_WILDCARDS => {
    known   => __('Known files (*.stl, *.obj, *.amf, *.xml)|*.stl;*.STL;*.obj;*.OBJ;*.amf;*.AMF;*.xml;*.XML'),
    stl     => __('STL files (*.stl)|*.stl;*.STL'),
    obj     => __('OBJ files (*.obj)|*.obj;*.OBJ'),
    amf     => __('AMF files (*.amf)|*.amf;*.AMF;*.xml;*.XML'),
    ini     => __('INI files *.ini|*.ini;*.INI'),
    gcode   => __('G-code files (*.gcode, *.gco, *.g)|*.gcode;*.GCODE;*.gco;*.GCO;*.g;*.G'),
    svg     => __('SVG files *.svg|*.svg;*.SVG'),
};
...
sub do_slice {
    ...
    eval {
        ...
        if ($copies > 1) {
            my $confirmation = Wx::MessageDialog->new($self, __x("Are you sure you want to slice {copies} copies?", copies => $copies),
        ...
        my $input_file;
        if (!$params{reslice}) {
            my $dialog = Wx::FileDialog->new($self, __('Choose a file to slice (STL/OBJ/AMF):'), $dir, "", MODEL_WILDCARD, wxFD_OPEN | wxFD_FILE_MUST_EXIST);
            ...
            if (!defined $last_input_file) {
                Wx::MessageDialog->new($self, __("No previously sliced file."),
                                       __('Error'), wxICON_ERROR | wxOK)->ShowModal();
                return;
            }
            if (! -e $last_input_file) {
                Wx::MessageDialog->new($self, __x("Previously sliced file ({last_input_file}) not found.", last_input_file => $last_input_file),
        ...
        if ($params{reslice}) {
            ...
            my $dlg = Wx::FileDialog->new($self, __('Save ' . ($params{export_svg} ? 'SVG' : 'G-code') . ' file as:'), dirname($output_file),
        ...
        $process_dialog = Wx::ProgressDialog->new(__('Slicing…'), __x("Processing {input_file_basename}…", input_file_basename => $input_file_basename), 
            100, $self, 0);
        ...
        my $message = __x("{input_file_basename} was successfully sliced", input_file_basename=> $input_file_basename);
        if ($print->processing_time) {
            $message .= __(' in');
            my $minutes = int($print->processing_time/60);
            $message .= sprintf __(" %d minutes and"), $minutes if $minutes;
            $message .= sprintf __(" %.1f seconds"), $print->processing_time - $minutes*60;
        }
        $message .= ".";
        &Wx::wxTheApp->notify($message);
        Wx::MessageDialog->new($self, $message, __('Slicing Done!'), 
...
sub export_config {
    ...
    my $dlg = Wx::FileDialog->new($self, __('Save configuration as:'), $dir, $filename, 
...
sub load_config_file {
    ...
    if (!$file) {
        ...
        my $dir = $last_config ? dirname($last_config) : $Slic3r::GUI::Settings->{recent}{config_directory} || $Slic3r::GUI::Settings->{recent}{skein_directory} || '';
        my $dlg = Wx::FileDialog->new($self, __('Select configuration to load:'), $dir, "config.ini", 
...
sub combine_stls {
    ...
    {
        my $dlg_message = __('Choose one or more files to combine (STL/OBJ)');
        while (1) {
    ...
    my $output_file = $input_files[0];
    {
        ...
        my $dlg = Wx::FileDialog->new($self, __('Save multi-material AMF file as:'), dirname($output_file),
...
sub check_unsaved_changes {
    ...
    if (@dirty) {
        ...
        my $confirm = Wx::MessageDialog->new($self, __x("You have unsaved changes ({titles}). Discard changes and continue anyway?", titles => $titles),
